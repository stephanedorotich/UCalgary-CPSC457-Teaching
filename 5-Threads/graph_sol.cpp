...
std::vector<std::string> detect_cycle(struct Graph & g) {
    ...
    for (auto [key, val] : g.out_counts) {
        if (val == 0) zero_outs.push_back(key);
    }
    ...
    while (!zero_outs.empty()) {
        node = zero_outs.back();
        zero_outs.pop_back();
        for (auto i : g.adj_list[node]) {
            counts_cp[i]--;
            if (counts_cp[i] == 0) zero_outs.push_back(i);
        }
    }
    ...
    for (auto [key, val] : counts_cp) {
        if (val != 0  && i.back() != '*') cycle.push_back(key);
    }
